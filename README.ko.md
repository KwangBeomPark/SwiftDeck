...
- **⌨️ 반복적 SQL 쿼리 및 매크로 자동화**: 재무 데이터 조회를 위해 자주 사용하는 길고 복잡한 SQL 쿼리나 자주 쓰는 AI프롬프트를 단축키 하나로 호출하여 자동 타자 및 엔터까지 일사천리로 수행합니다.
- **📧 정형화된 이메일 템플릿 및 보고 문구**: 자금 일보 보고, 부서 간 예산 실적 조 조정 요청 등 매일 반복되는 긴 이메일 양식 또는 특수 기호 등을 단축 지정어(Hotstring)로 0.1초 만에 확장하여 입력합니다.

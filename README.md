...
3. Unzip and double-click **`SwiftDeck.exe`**. A black lightning bolt icon will activate in your Windows system tray, and it's ready to use instantly!
   - *💡 **Team Productivity Tip**: A manager can configure commonly used reporting formats (Prompts) or shared network paths (Folders) in the dashboard. By distributing the generated `.ini` configuration files to colleagues in the same folder, you can instantly **standardize the workflow across the entire department**.*
